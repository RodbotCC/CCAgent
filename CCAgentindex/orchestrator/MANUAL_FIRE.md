...
9. **Contractual Commitments Hard Gate (CRITICAL)** — see
   `comeketo-inbox/references/guardrails.md §10c`. Scan BOTH the plan body
   in `05_seven_day_plan.md` AND the composed draft for trigger phrases:
   `free`, `no charge`, `no fee`, `no cost`, `waive`/`waived`/`waiving`,
   `complimentary`, `comp'd`, `on us`, `on the house`, `won't charge`,
   `cover the` / `we'll cover` / `covered for`, `throwing in`, `bonus`,
   `up to N people` / `entire party` / `for the whole party`,
   `guaranteed` / `lock in` / `price hold` / `rate hold`, `discount`,
   `% off`, `$N off`. Match case-insensitively. If ANY trigger phrase
   appears AND the same paragraph does NOT carry an `[APPROVED BY ANDRE — ...]`
   or `[APPROVED BY JAKE — ...]` tag: STOP. Surface `[COMMITMENT FLAG]` to
   André's DM with the lead, day, draft text, matched phrase, and
   approval question. **No 30-min auto-timeout.** Wait for explicit YES,
   a rewritten draft, or a kill instruction. Log to `state/runs/`.
   **Why:** the Brenda & Steve Day 1 fired a fee-waiver commitment ("waive
   the tasting fee for the entire party… up to 10 people, no charge") on
   2026-04-27 because no audit gate caught it. Money, headcount math,
   deliverables, and terms decisions never live inside a plan body without
   explicit operator authorization.
10. **Enrichment Use Boundary (CRITICAL)** — see
    `comeketo-inbox/references/guardrails.md §10d`. Extract every specific
    factual claim about the lead from the draft (industry, employer, role,
    location specifics, family members named, prior events, history
    claims). For each claim, grep `01_comms.md` for evidence the lead — or
    a sender on their behalf in this thread — has shared this fact. If a
    claim has NO supporting evidence in `01_comms.md` AND the plan paragraph
    does NOT carry an `[ENRICHMENT APPROVED — Andre/Jake ok'd this reference]`
    tag: STOP. Surface `[ENRICHMENT BOUNDARY]` to André's DM with the
    claim, draft text, and origin (which research stage file). Wait for
    rewrite or kill instruction. Acceptable evidence: inbound text/email
    from the lead, phone-call notes in `01_comms.md`, intake form
    fields, or prior outbound the lead engaged with that named the fact
    first. NOT acceptable: anything from `02_deep_dive.md`,
    `03_enrichment.md`, public records, LinkedIn, social media, common-
    knowledge assumptions, or third-party share (spouse/planner unless
    on-thread). **Allowed steering** uses what the lead said:
    "since you mentioned the tasting on Saturday…" **Not allowed surfacing**
    quotes enrichment: "Steve, I know you've been in food service a long
    time…" The second reads as surveillance even when the fact is true.
    **Why:** the Brenda & Steve Day 1 email opened with an enrichment-
    sourced industry reference. Per the phone transcript that fact was
    actually fine because it had been discussed verbally — but the system
    cannot know that without checking comms, and the rule must enforce
...
**Hard-gate distinction.** Audits 1–8 fail-skip. Audits 9 and 10 fail-PAUSE
— they require an explicit operator answer (YES, rewrite, or kill) before
the fire can proceed. There is no auto-timeout on a hard gate. A flagged
move stays flagged in André's DM until cleared.
...
| Audit fail (gates 1–8) | team channel | `⏭ <Name> Day <N> skipped — <reason>` |
| **Hard-gate hold (gate 9 — commitment)** | André's DM (urgent) | `[COMMITMENT FLAG] <Name> Day <N> — matched "<phrase>" in draft. <draft preview>. Authorize? Reply YES to fire, send a rewrite, or "kill".` — no auto-timeout |
| **Hard-gate hold (gate 10 — enrichment)** | André's DM (urgent) | `[ENRICHMENT BOUNDARY] <Name> Day <N> — claim "<fact>" is not in 01_comms.md (origin: <stage_file>). <draft preview>. Soften / rewrite / kill?` — no auto-timeout |
...
4. Pre-flight against `comeketo-inbox/references/guardrails.md` —
   ...
   §10c (Contractual Commitments Hard Gate — fee waivers / free /
   waive / no charge / scope expansions / price guarantees blocked
   without `[APPROVED BY ...]` tag), §10d (Enrichment Use Boundary —
   no specific lead-fact in the draft unless it traces to `01_comms.md`
   or carries `[ENRICHMENT APPROVED — ...]`).
