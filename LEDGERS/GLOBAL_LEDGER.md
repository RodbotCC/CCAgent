...
Last updated: 2026-04-29 (Phase 14 — Asset/Widget Map landed; cross-page widget catalog + API→page mapping; sitemap remains canonical per-page truth)
...
**Current preferred workflow:** one ledger at a time. Phases 1–14 (Global + Temporal Continuity + North Star + File Directory + Open Problems + Communications + Decisions + Box Ledger + Definition of Done + Box Bus Ledger + Source-of-Truth + Connections + File Contents + **Asset/Widget Map**) have landed. The Global Ledger Steward is the only runnable steward today; the other 5 packages live as drafts at `/Subagent Boxes/`. Audit Ledger was dropped from the build queue. Next ledger candidates: **Phase Ledger** (small, pairs with DoD §11; tier: global), **Page Ledgers** (one per UI route; tier: domain — start with `boxes` or `intake`), or **Settings Ledger** (tier: global; pairs with CONNECTIONS for the UI-configurable surface). See `LEDGERS/INDEX.md` for the full Phase 15+ order.
