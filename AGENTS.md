...
```json
{"ts":"<ISO8601>","kind":"delegation_write","actor":"Codex","request_id":"<dl_id>","action":"<verb>","target":"<relative_path>","notes":"<one sentence>"}
```
...
### 2) Codex subprocess side (delegations)

Delegations spawn `Codex -p` as a subprocess and inherit the user's global MCP config. To make Pieces available to Codex (and therefore to every Comeketo Agent delegation), register it once at the user scope:

```bash
Codex mcp add --transport http pieces http://localhost:39300/model_context_protocol/2025-03-26/mcp
```

Falls back to SSE if HTTP isn't supported by the server build:

```bash
Codex mcp add --transport sse pieces http://localhost:39300/model_context_protocol/2024-11-05/sse
```

**Manage:**

```bash
Codex mcp list             # confirm pieces is listed
Codex mcp get pieces       # show the registered config
Codex mcp remove pieces    # uninstall
```
...
- **Transport mismatch** → try switching `--transport http` ↔ `--transport sse` when registering with Codex.
- **VS Code extension not seeing tools** → restart VS Code after `Codex mcp add`.
- **Config locations** → user scope `~/.Codex.json`; project scope `.mcp.json` at project root.
